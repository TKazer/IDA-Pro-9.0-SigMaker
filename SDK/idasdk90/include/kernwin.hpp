...
/// \defgroup CVLF_ Custom viewer location-retrieving flags
/// passed as 'flags' parameter to get_custom_viewer_location()
///@{
#define CVLF_USE_MOUSE  0x1 ///< Fetch the location from the mouse, instead of caret in the listing
///@}
...
enum ui_notification_t
{
  ...
  ui_obsolete_get_custom_viewer_location, // ui_get_custom_viewer_location_2
  ...
  ui_get_custom_viewer_location_2,
                          ...
                          ///< \param custom_viewer (TWidget *)
                          ///< \param flags (uint32) or'ed combination of CVLF_* values

  ui_get_chooser_rows,    ///< ui: see get_chooser_rows()
                          ///< \param out (chooser_row_info_vec_t *) column data for each returned row
                          ///< \param chooser_caption (const char *)
                          ///< \param what (size_t) row number, or one of the GCRF_ flags
  ...
  ui_open_builtin2,       ///< ui: open a window of a built-in type. see \ref ui_open_builtin_funcs
                          ...
                          ///< \param arg0 the first argument (e.g., ordinal for BWN_TILVIEW)
                          ///< \param arg1 the second argument (e.g., a tif_cursor_t * for BWN_TILVIEW)
...
// Return true if this is ida running as library
const int64 IDALIB_API_MAGIC = 0x001DA11B00000000LL;
inline bool is_ida_library(char *path, size_t pathsize, void** handle)
{
...
#define MAX_PLACE_LNNUM INT_MAX
...
/*! \defgroup simpleline Simpleline interface
  ...
  into it and create a custom ida viewer (::create_custom_viewer).
                                                                     \code
    strvec_t lines;
    // fill it with lines...
...
public:
  hexplace_t(ea_t _ea, int ln) : idaplace_t(_ea, ln), sol(_ea) {}
...
inline bool is_tif_cursor_header(tif_cursor_t c) { return c == TIF_CURSOR_HEADER; }
inline bool is_tif_cursor_footer(tif_cursor_t c) { return c == TIF_CURSOR_FOOTER; }
inline bool is_tif_cursor_index(tif_cursor_t c) { return c != TIF_CURSOR_HEADER && c != TIF_CURSOR_FOOTER; }

//-------------------------------------------------------------------------
class tiplace_t : public place_t
{
public:
  uint32 ordinal = 0;
  ...
  tiplace_t() {}
  tiplace_t(uint32 ord, tif_cursor_t c, int _lnnum) : ordinal(ord), cursor(c)
  {
    lnnum = _lnnum;
  }
  bool valid_ord() const { return ordinal != 0; }
  void reset() { ordinal = 0; cursor = TIF_CURSOR_HEADER; lnnum = 0; }

  bool is_header() const { return cursor == TIF_CURSOR_HEADER; }
  bool is_footer() const { return cursor == TIF_CURSOR_FOOTER; }
  bool is_index()  const { return cursor < TIF_CURSOR_HEADER; }

  void set_header() { cursor = TIF_CURSOR_HEADER; }
  void set_footer() { cursor = TIF_CURSOR_FOOTER; }

  define_place_virtual_functions(tiplace_t);

  /// for structs: calculate the offset that corresponds to the tiplace.
  /// \param ud        pointer to user-defined context data. Is supplied by ::linearray_t
  /// \param p_udmidx  place to return the index of the current udt member, if any.
  ///                  if there is no member at the current offset, return -1
  /// \param p_bitoff  place to return the bit offset of the item from the beginning of the bit bucket
  ///                  if there no bitfields, return -1
  ...
  virtual void set_index_by_offset(void *ud, uint64 offset) newapi;
...
//-------------------------------------------------------------------------
// Represents a range in a line to be displayed in a listing (or graph).
// All coordinates are in codepoints (i.e., not UTF-8 bytes); color
// tags are ignored.
// In other words, coordinates are in "real, visible" glyphs.
typedef int cpidx_t; // codepoint index
typedef int cplen_t; // codepoint length
...
struct line_section_t
{
  ...
  bool is_open() const { return length < 0; }
  bool is_closed() const { return !is_open(); }
  bool valid() const { return is_closed(); }
};
DECLARE_TYPE_AS_MOVABLE(line_section_t);
typedef qvector<line_section_t> line_sections_t;

//-------------------------------------------------------------------------
// A tagged section in a line.
// Note: multiple sections with different tags can overlap.
// For example, the portion of text `[ebp+char_in_main]`:
// <on><opnd1><on><symbol>[<off><symbol><on><reg>ebp<off><reg><on><symbol>+<off><symbol><on><locname>char_in_main<off><locname><on><symbol>]<off><symbol><off><opnd1>
// ^________________________________________________________________________________________________________________________________________________________________^       // a `COLOR_OPND1` section
//           ^_________________________^                                                                                                                                    // a `COLOR_SYMBOL` section
//                                      ^____________________^                                                                                                              // a `COLOR_REG` section
// etc...
struct tagged_line_section_t : public line_section_t
{
  struct
  {
    int text_start = -1; // points right after <on><tag>
    int text_end = -1;   // points right before <off><tag>
  } byte_offsets;
  color_t tag = 0;
  ...
  bool valid() const
  {
    return line_section_t::valid()
        && tag > 0
        && byte_offsets.text_start > -1
        && byte_offsets.text_end >= byte_offsets.text_start;
  }

  bool valid_in(const qstring &in) const
  {
    return valid()
        && byte_offsets.text_start <= in.length()
        && byte_offsets.text_end <= in.length();
  }

  bool substr(qstring *out,
        const qstring &in,
        const tagged_line_section_t *end = nullptr) const
  {
    ...
    {
      out->qclear();
      int e = byte_offsets.text_end;
      if ( end != nullptr && end->byte_offsets.text_start < e )
        e = end->byte_offsets.text_start;
      out->append(in.c_str() + byte_offsets.text_start,
                  e - byte_offsets.text_start);
    }
    return ok;
  }
...
//-------------------------------------------------------------------------
// The list of tagged sections corresponding to a line of text in a listing
struct tagged_line_sections_t : public tagged_line_section_vec_t
{
  void sections_at(
  ...
  //                    ^ r2                          ^ r3
  // nearest_at(<any p>) --> r1 (because r2 and r3 are zero-length)
  ...
  // ^___ r1 ________________________________________________________________________________^
  //              ^ p1  ^ r2               ^ p2       ^ r3                  ^ p3
  // nearest_before(r1, p1, COLOR_ADDR) --> nullptr
  // nearest_before(r1, p2, COLOR_ADDR) --> r2 (zero-length)
  // nearest_before(r1, p3, COLOR_ADDR) --> r3 (zero-length)
  // nearest_after (r1, p1, COLOR_ADDR) --> r2 (zero-length)
  // nearest_after (r1, p2, COLOR_ADDR) --> r3 (zero-length)
  // nearest_after (r1, p3, COLOR_ADDR) --> nullptr
  const tagged_line_section_t *nearest_before(
        const tagged_line_section_t &range,
        cpidx_t start,
        color_t tag=0) const
  {
    const tagged_line_section_t *nearest = nullptr;
  ...
  const tagged_line_section_t *nearest_after(
        const tagged_line_section_t &range,
        cpidx_t start,
        color_t tag=0) const
  {
...
struct listing_location_t
{
  ...
  listing_location_t() : cb(sizeof(*this)) {}
};
...
#define BWN_TILVIEW       10 ///< local types
...
#define BWN_DISASM        27 ///< disassembly views
#define BWN_DUMP          28 ///< hex dumps
#define BWN_NOTEPAD       29 ///< notepad
#define BWN_OUTPUT        30 ///< the text area, in the output window
#define BWN_CLI           31 ///< the command-line, in the output window
#define BWN_WATCH         32 ///< the 'watches' debugger window
#define BWN_LOCALS        33 ///< the 'locals' debugger window
#define BWN_STKVIEW       34 ///< the 'Stack view' debugger window
#define BWN_CHOOSER       35 ///< a non-builtin chooser
#define BWN_SHORTCUTCSR   36 ///< the shortcuts chooser (Qt version only)
#define BWN_SHORTCUTWIN   37 ///< the shortcuts window (Qt version only)
#define BWN_CPUREGS       38 ///< one of the 'General registers', 'FPU register', ... debugger windows
#define BWN_SO_STRUCTS    39 ///< the 'Structure offsets' dialog's 'Structures and Unions' panel
#define BWN_SO_OFFSETS    40 ///< the 'Structure offsets' dialog's offset panel
#define BWN_CMDPALCSR     41 ///< the command palette chooser (Qt version only)
#define BWN_CMDPALWIN     42 ///< the command palette window (Qt version only)
#define BWN_SNIPPETS      43 ///< the 'Execute script' window
...
#define BWN_SRCPTHMAP_CSR 52 ///< "Source paths..."'s path mappings chooser
#define BWN_SRCPTHUND_CSR 53 ///< "Source paths..."'s undesired paths chooser
#define BWN_UNDOHIST      54 ///< Undo history
#define BWN_SNIPPETS_CSR  55 ///< the list of snippets in the 'Execute script' window
#define BWN_SCRIPTS_CSR   56 ///< the "Recent scripts" chooser
#define BWN_BOOKMARKS     57 ///< a persistent 'Bookmarks' widget
#define BWN_TILIST        58 ///< a type listing widget
...
#define IWID_TILVIEW       (1ULL << BWN_TILVIEW      ) ///< local types
...
#define IWID_DUMP          (1ULL << BWN_DUMP         ) ///< hex dumps
...
#define IDA_DEBUG_RANGECB       0x08000000      ///< range-based entities like segments, functions and so on
...
/// \defgroup GCRF_ Flags for get_chooser_rows().
///@{
#define GCRF_HIGH_BIT  (0x8000000000000000ULL) // used to be:
                                               // (size_t(1) << (8 * sizeof(size_t) - 1))
                                               // but SWIG won't swallow the expression
#define GCRF_HEADER    (GCRF_HIGH_BIT | 0)  ///< Return header texts
#define GCRF_SELECTION (GCRF_HIGH_BIT | 1)  ///< Return selected rows
#define GCRF_CURRENT   (GCRF_HIGH_BIT | 2)  ///< Return the current row
...
public:
  ...
  bool has_widget_lifecycle() const { return (flags & CH_KEEP) == 0; }
...
struct chooser_t : public chooser_base_t
{
  ...
  /// Map an a dirtree_t inode to the index
  /// This will be used only if CH_HAS_DIRTREE and CH2_HAS_INODE2INDEX are specified
  /// Implementing this function will result in increase overall performance
  /// especially for big choosers.
  /// \param inode inode of the item
  /// \return the index
  virtual size_t idaapi inode_to_index(inode_t /*inode*/) const newapi { return size_t(-1); }
...
struct exec_request_t
{
  ...
  ssize_t code = 0;   ///< temporary location, used internally
  ...
  /// Destructor
  virtual ~exec_request_t() { qsem_free(sem); sem = nullptr; code = 0; }
};
...
#define ACF_HAS_FIELD_DIRTREE_SELECTION (1 << 2) ///< 'dirtree_selection' field is present
#define ACF_HAS_SOURCE                  (1 << 3) ///< 'source' field is present
#define ACF_HAS_TYPE_REF                (1 << 4) ///< 'type_ref' field is present
///@}
  ...
  void reset()
  {
    ...
    //
    cur_flags = ACF_HAS_FIELD_DIRTREE_SELECTION | ACF_HAS_SOURCE | ACF_HAS_TYPE_REF;
    cur_ea = BADADDR;
    cur_value = BADADDR;
...
inline int get_icon_id_by_name(const char *icon_name) { return callui(ui_get_icon_id_by_name, icon_name).i; }
...
inline bool get_custom_viewer_location(
        ...
        bool mouse=false)
{
  return callui(ui_obsolete_get_custom_viewer_location, out, custom_viewer, mouse).cnd;
}
...
/// \param custom_viewer the listing widget
/// \param flags or'ed combination of CVLF_* values
inline bool get_custom_viewer_location(
        listing_location_t *out,
        TWidget *custom_viewer,
        uint32 flags=0)
{
  return callui(ui_get_custom_viewer_location_2, out, custom_viewer, flags).cnd;
}
...
{
  return (TWidget *) callui(ui_open_builtin2, BWN_TILVIEW, ordinal, cursor).vptr;
}
...
inline ea_t choose_stkvar_xref(func_t *pfn, tid_t srkvar_tid)
{
  ea_t ea;
...
/// \param[out] out    the selected enum type
/// \param title       chooser title
...
inline bool choose_enum(tinfo_t *out, const char *title, uint32 default_ord)
{
  return callui(ui_choose, chtype_enum, out, title, default_ord).cnd;
}
...
inline bool choose_enum_by_value(
        tinfo_t *out,
        const char *title,
        uint32 default_ord,
        uint64 value,
        ...
        uchar *serial)
{
  return callui(ui_choose, chtype_enum_by_value_and_size, out, default_ord, title, value, nbytes, serial).cnd;
}
...
/// \param[out] out    the selected structure type
/// \param[in]  title  chooser title
/// \retval true   the selected type is in OUT
/// \retval false  nothing was selected

inline bool choose_struct(tinfo_t *out, const char *title)
{
  return callui(ui_choose, chtype_struct, out, title).cnd;
}
...
inline bool get_chooser_rows(
        chooser_row_info_vec_t *out,
        const char *chooser_caption,
        size_t what)
{
  return callui(ui_get_chooser_rows, out, chooser_caption, what).cnd;
}
...
/*! \defgroup FORM_C ask_form()/open_form()
  ...
  e - type editor                                 ::type_editor_ctrl_t *
...
#define DEF_STR_FIELD_METHOD(NAME            )                       \
  ...
  // get/set tabbed widget current tab
  DEF_FIELD_METHOD(current_tab, int)
  // get/set tabbed type editor
  DEF_FIELD_METHOD(type, type_editor_ctrl_t)
...
#undef DEF_STR_FIELD_METHOD
  ...
  /// Currently supports 'multi line text control' and 'chooser' field types.
  /// GUI version only.
  /// \return success
  ...
  inline bool set_tab_visible(int fid, int tab, bool visible)
  {
    return set_tab_attribute(fid, tab, ta_visible, &visible);
  }
  inline bool is_tab_visible(bool *out, int fid, int tab)
  {
    return get_tab_attribute(out, fid, tab, ta_visible);
  }
  inline bool set_tab_enabled(int fid, int tab, bool enabled)
  {
    return set_tab_attribute(fid, tab, ta_enabled, &enabled);
  }
  inline bool is_tab_enabled(bool *out, int fid, int tab)
  {
    return get_tab_attribute(out, fid, tab, ta_enabled);
  }
};
...
#define HIST_IDENT  4           ///< identifiers. usually CPU register names are forbidden
...
#define HIST_IDENT2 9           ///< identifiers, including CPU register names
///@}
...
/// and allows the user to correct it. CPU register names are usually forbidden.
/// \param str      qstring to fill. Can contain the default value. Cannot be nullptr.
/// \param format   printf() style format string with the question
/// \return false if the user cancelled the dialog, otherwise returns true.
...
/// Display a dialog box and wait for the user to input an identifier.
/// If the user enters a non-valid identifier, this function displays a warning
/// and allows the user to correct it. CPU register names are permitted.
/// \param str      qstring to fill. Can contain the default value. Cannot be nullptr.
/// \param format   printf() style format string with the question
/// \return false if the user cancelled the dialog, otherwise returns true.

AS_PRINTF(2, 3) inline bool ask_ident2(qstring *str, const char *format, ...)
{
  va_list va;
  va_start(va, format);
  bool result = vask_str(str, HIST_IDENT2, format, va);
  va_end(va);
  return result;
}
...
struct addon_info_t
{
  ...
  const char *freeform;     //< any string, e.g. "Copyright (c) 2007-2024 Hex-Rays"
...
#define IS_TYPE_EDITOR_CTRL_T(v) (std::is_base_of<type_editor_ctrl_t, std::remove_reference<decltype(v)>::type>::value)
